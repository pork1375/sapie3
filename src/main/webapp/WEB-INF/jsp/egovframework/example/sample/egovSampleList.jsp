	  ...
	  $.ajax({
		  ...
		  success: function(wall) {
			  ...
			  for(var i = 0; i < wall['qwer'].length; i++) {
					...
					html += '<td>'+wall.qwer[i].priceClose.toLocaleString()+'</td>';
					html += '<td>'+wall.qwer[i].priceAvg5.toLocaleString()+'</td>';
					html += '<td>'+wall.qwer[i].priceAvg10.toLocaleString()+'</td>';
					html += '<td>'+wall.qwer[i].priceAvg20.toLocaleString()+'</td>';
					html += '<td>'+wall.qwer[i].priceAvg60.toLocaleString()+'</td>';
					html += '<td>'+wall.qwer[i].volume.toLocaleString()+'</td>';
					html += '<td>'+wall.qwer[i].volumeAvg5.toLocaleString()+'</td>';
					html += '<td>'+wall.qwer[i].volumeAvg20.toLocaleString()+'</td>';
					html += '<td>'+wall.qwer[i].volumeAvg60.toLocaleString()+'</td>';
					html += '</tr>';
				}
...
/* 		    $.ajax({
 ...
 function gridtable(bio) {
       // 변수 선언
       var i, max, myData = $("#list2");

       // grid 설정
       $("#list2").jqGrid({
           datatype: 'json',
           height: 'auto',
           colNames:['itemId','dealDate','itemName', 'priceOpen', 'priceHigh','priceLow','priceClose','compared3','fluctuation','volume'],
           colModel:[
               {name:'itemId',index:'itemId', width:55},
               {name:'dealDate',index:'dealDate', width:55},
               {name:'itemName',index:'itemName', width:55},
               {name:'priceOpen',index:'priceOpen', width:55},
               {name:'priceHigh',index:'priceHigh', width:55},
               {name:'priceLow',index:'priceLow', width:55},
               {name:'priceClose',index:'priceClose', width:55},
               {name:'compared3',index:'compared3', width:55},
               {name:'fluctuation',index:'fluctuation', width:55},
               {name:'volume',index:'volumeAvg60', width:55}
           ],
           multiselect: true,
          	rowNum:10,
           	rowList:[10,20,30],
            ...
            /* beforeSelectRow: function(rowid, e) {	// 클릭 시 이벤트 주기
              	$('#history')
                	.append('beforeSelectRow() - rowid: ', rowid, 
                    '\t\te: ', e + '<br/>')
                return false; // true 반환하지 않으면 다음 이벤트가 수행되지 않음.
       ...
       // 데이터 추가
       for( i=0, max = myData['rows'].length ; i<=max ; i++ ) {
    	   jQuery("#list2").jqGrid('addRowData', i+1, myData['rows'][i]);
       }
   }
 */
 ...
 $(document).ready(function () {
		...
		datatype: "json",
		 colModel: [
			 { label: '종목코드' 	, name: 'itemId' , key:true },
			 { label: '종목명'	, name: 'itemName' },
			 { label: '시가'		, name: 'priceOpen' },
			 { label: '고가'		, name: 'priceHigh' },
			 { label: '저가'		, name: 'priceLow' },
			 { label: '종가'		, name: 'priceClose' },
			 { label: '전일대비'	, name: 'compared3' },
			 { label: '등락률'	, name: 'fluctuation' },
			 { label: '거래량'	, name: 'volume' },
			 { label: '거래일'	, name: 'dealDate' },
		],
		// http://1004lucifer.blogspot.com/2019/04/jqgrid_65.html        // 옵션 많이 모아놓은곳 참조
		viewrecords: true,		// 데이터가 없다면 오른쪽에 보여질 문자열??? 에러뜨거나 안나오는걸 어떻게 확인해
		width: 780,				// 가로 길이
		height: 'auto',			// 높이 오토
		rowNum: 10,				// 기본겂으로 10개를 보여준다
		rowList:[10,20,30],		// 리스트에 보여주는 갯수
		loadonce: true,			// 	true로 설정된경우 그리드는 서버에서 데이터를 한번만 로드한다. 등등   안써주면 pager 안됨 
		pager: "#jqGridPager",	// 페이징
		rownumbers: true,		// 리스트 번호
		hoverrows: true,		// false로 설정 시 그리드데이터 row위에 마우스를 올리면 hover 효과가 없어진다.
		// multiselect: true,	// 다중체크
		
		/*
		beforeSelectRow: function(rowid, e) {	// 클릭 이벤트 테스트
		  	$('#history').append('beforeSelectRow() - rowid: ', rowid, 
		        '\t\te: ', e  + '<br/>')
		        
		    return true // true 반환하지 않으면 다음 이벤트가 수행되지 않음. 매우매우 중요!!!
		  },
		  */
		onCellSelect : function(rowid, iCol) {
			console.log("그리드리스트클릭");
			var row = $("#jqGrid").getRowData(rowid);
			var itemId = row.itemId;
			
			call(itemId);	// 클릭 리스트 불러온다.
		}
	});
...
<!-- <div id="history" style="width:780px;"></div> -->

<table id="list2"></table><!-- ajax로 만든 리스트 -->
<div id="pager2"></div>
<!-- <button id="export">프프프프</button> -->
